...
Execute `msmodelslim quant -h`. For deeper parameter details, read the local documentation or source code located in the `msmodelslim` installation path resolved in Step 1. Do not hallucinate parameters.
...
Use `LS` and `Read` to explore the `example/` or `docs/  directories within the resolved msmodelslim installation path. Find a script with the closest architectural match.
