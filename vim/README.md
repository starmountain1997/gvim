...
1. **Git 操作** - 执行 git 命令进行版本控制
1. **编译构建** - 运行编译或构建命令
1. **文件管理** - 使用 shell 命令进行文件操作
...
1. **Git Hook**：在提交前自动运行格式化检查
1. **IDE 集成**：在 vim/VSCode 等编辑器中启用保存时格式化
1. **CI/CD**：在持续集成中检查代码格式
...
1. **正常开发**: 打开多个文件、分割窗口、调整布局
1. **结束工作**: 直接关闭 vim，会话状态已自动保存
1. **继续工作**: 重新打开 vim，自动恢复到之前的状态
...
1. **版本控制**: 将 `Session.vim` 添加到 `.gitignore`，避免提交个人工作状态
1. **定期清理**: 项目完成后使用 `:Obsess!` 停止会话，删除会话文件
...
1. 在远程服务器上使用 vim 编辑文件
1. 复制的内容会通过 OSC 52 序列传输到本地终端
1. 内容自动进入本地系统的剪贴板
...
1. **检查长度限制**: 某些终端对 OSC 52 序列长度有限制
1. **SSH 配置**: 确保 SSH 连接允许转义序列通过
...
1. **X11**: 在 Linux 桌面环境中使用 `xclip` 或 `xsel`
1. **Mac**: 在 macOS 中使用 `pbcopy` 和 `pbpaste`
1. **Windows**: 在 Windows 中使用 `clip.exe`
...
1. **设置断点**: 在想要暂停的行按 F9 设置断点
1. **开始调试**: 按 F5 开始调试会话
1. **单步执行**: 使用 F10/F11 逐步执行代码
1. **查看变量**: 在变量窗口中查看当前变量值
1. **继续执行**: 按 F5 继续到下一个断点或程序结束
...
1. **验证配置文件**: 检查 `.vimspector.json` 语法是否正确
1. **环境变量**: 确保 PATH 包含调试器可执行文件

##### 断点不生效

1. **检查文件路径**: 确保文件路径与配置中的路径匹配
1. **代码可执行**: 确保代码是可执行的（语法正确）
1. **调试模式**: 某些调试器需要特定的调试标志

##### 变量显示异常

1. **刷新变量**: 尝试刷新变量窗口
1. **作用域问题**: 检查变量是否在当前作用域内
1. **调试器版本**: 更新到最新版本的调试器适配器
...
1. **上下文提取**: 自动提取当前光标所在位置的父级代码块
1. **窗口管理**: 在编辑区域顶部创建独立的上下文显示区域
1. **实时同步**: 滚动时动态更新上下文内容
...
1. **检查配置**: 确认配置参数设置合理
1. **更新插件**: 确保使用最新版本的 context.vim

#### 最佳实践

1. **合理设置行数**: 根据屏幕大小调整 `g:context_max_lines`
1. **配合折叠功能**: 与 Vim 的折叠功能结合使用，效果更佳
1. **快捷键绑定**: 可以绑定快捷键快速切换上下文显示
